...
namespace Eco.Mods.TechTree
{
    ...
    public partial class PickaxeItem : ToolItem
    {
        ...
        private static SkillModifiedValue caloriesBurn = CreateCalorieValue(
            20,
            typeof(MiningSkill),
            typeof(PickaxeItem)
        );
        ...
        [SyncToView]
        public virtual float Damage => 1.0f; // damage per tier level

        [SyncToView]
        public override IDynamicValue Tier => base.Tier; // tool tier, overriden to have SyncToView only for pickaxe
        ...
        public override InteractResult OnActLeft(InteractionContext context)
        {
            if (context.HasBlock && context.Block.Is<Minable>())
            {
                ...
                var item =
                    context.Block is IRepresentsItem
                        ? Item.Get((IRepresentsItem)context.Block)
                        : null;

                var totalDamageToTarget = user.BlockHitCache.MemorizeHit(
                    context.Block.GetType(),
                    context.BlockPosition.Value,
                    this.Tier.GetCurrentValue(context.Player.User) * this.Damage
                );
                if (context.Block.Get<Minable>().Hardness <= totalDamageToTarget)
                {
                    var result = AtomicActions.DeleteBlockNow(
                        this.CreateMultiblockContext(context, false),
                        spawnRubble: false
                    ); // Use the tool, but do not trigger an AddExperience call for destroying blocks.
                    ...
                    if (result.Success)
                    {
                        ...
                        {
                            pickupItems =
                                (miningSkill.Level >= AutoCollectLevel)
                                    ? user.Inventory.Carried.TryAddItems(item.GetType(), 4).Success
                                    : false;
                        }
                        ...
                        {
                            var forced =
                                (miningSkill.Level >= LuckyBreakLevel && LuckyBreak) ? 4 : -1;
                            if (
                                RubbleObject.TrySpawnFromBlock(
                                    context.Player,
                                    context.Block.GetType(),
                                    context.BlockPosition.Value,
                                    forced
                            ...
                            )
                            {
                                this.AddExperience(
                                    user,
                                    1f,
                                    new LocString(Localizer.Format("mining") + addition)
                                ); // Add experience based on the tool's experience rate (altered in the EcoTechTree.csv).
                    ...
                    return (InteractResult)
                        AtomicActions.UseToolNow(this.CreateMultiblockContext(context, false)); // Use the tool, but do not trigger an AddExperience call for damaging blocks.
            }
