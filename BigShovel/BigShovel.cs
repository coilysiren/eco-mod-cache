...
namespace Eco.Mods.TechTree
{
    ...
    public partial class BigShovelItem : ToolItem
    {
        private static SkillModifiedValue caloriesBurn = CreateCalorieValue(
            ...
            typeof(BigShovelItem)
        );
        private static IDynamicValue skilledRepairCost = new ConstantValue(1);
        private static IDynamicValue tier = new ConstantValue(0);

        public override GameActionDescription DescribeBlockAction =>
            new(Localizer.DoStr("dig up"), Localizer.DoStr("digging up"));
        public override IDynamicValue CaloriesBurn => caloriesBurn;
        public override ClientPredictedBlockAction LeftAction =>
            ClientPredictedBlockAction.PickupBlock;
        ...
        public override InteractResult OnActLeft(InteractionContext context)
        {
            ...
            if (Durability == 0)
                return InteractResult.FailureLoc(
                    $"Your Tool is Broken, You Need to Repair It First"
                );

            // Fallback if not enough room in carrying stack
            if (context.Player.User.Carrying.Quantity >= this.MaxTake)
                return InteractResult.FailureLoc($"There is not enough room to pick this up.");
            ...
            if (context.HasBlock && context.Block.Is<Diggable>())
            {
                ...
                if (context.Block is PlantBlock)
                    return (InteractResult)
                        AtomicActions.DestroyPlantNow(
                            this.CreateMultiblockContext(context),
                            harvestTo: context.Player?.User.Inventory
                        );

                return (InteractResult)
                    AtomicActions.DeleteBlockNow(
                        context: this.CreateMultiblockContext(context),
                        harvestPlantsAbove: World.World
                            .GetBlock(context.BlockPosition.Value + Vector3i.Up)
                            .Is<Diggable>(),
